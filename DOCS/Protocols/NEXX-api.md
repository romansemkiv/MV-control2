...
| Максимум параметрів в одному batch-запиті | 40 |

### 11.2 Важливі примітки

1. **Batch ліміт** - запити `/EV/GET/parameters/` та `/EV/SET/parameters/` підтримують не більше **40 параметрів** в одному запиті. При перевищенні API повертає помилку: `"unable to retrieve parameter value - no more than 40 parameters in one request."`
